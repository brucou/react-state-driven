...
White-box testing of the configured component hence involves testing teach of the aforementionned
...
We want to test the component in a way that generates confidence about the behaviour of the 
component, finds errors along the way, but at the same time resists well to changes in 
implementation.
...
machine modelization. This means that any tests of the state machine model will still remain 
valid, even under a change of the implementation of the component. In particular, even when the 
...
of input sequence vs. the expected output sequence. The test space for such test scenari is 
...
known about the system at hand can also help identify the valuable section of the test space to 
put under test. 

The [react-state-driven library]() offers a test generator which can be configured according to 
several model coverage criteria. Such or similar test generators may simplify further the testing
 procedure. 
...
against the state machine, we can afford to do enact only those tests which are useful for 
